...
ADR-001 made agent-native repository surfaces authoritative and deferred plugin packaging until the workflow proved stable. Codexicon can create a fresh repository, but an established project has no deterministic way to inspect compatibility, adopt only missing harness pieces, diagnose partial or stale configuration, or apply a later Codexicon release without manually guessing which files remain template-owned.
...
Codexicon will use dependency-free current verification scripts and a versioned source manifest, `.gemini/config.toml`.
...
- State lock files (formerly `.codexicon.lock.json`) do not exist; baseline hashes are not recorded this way.
...
| Packaged CLI or agent plugin | Central distribution and version discovery | New install, trust, compatibility, and publication lifecycle before the local protocol is proven | ADR-001's plugin revisit signal is not met |
...
- Source contract: `.gemini/config.toml`, schema version 1.
- Installed contract: State locks do not exist.
- Transaction state: Transaction journals do not exist.
- User interfaces: `inspect`, `adopt`, `doctor`, `update`, `verify`, `checkpoint`, `resume`, and `install-git-hooks`.
- Template validation checks the manifest, documented Gemini config, hook registration, POSIX modes/EOL policy, and skill budget.
