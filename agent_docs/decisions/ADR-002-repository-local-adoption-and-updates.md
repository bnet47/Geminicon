...
ADR-001 made agent-native repository surfaces authoritative and deferred plugin packaging until the workflow proved stable. Geminicon can create a fresh repository, but an established project has no deterministic way to inspect compatibility, adopt only missing harness pieces, diagnose partial or stale configuration, or apply a later Geminicon release without manually guessing which files remain template-owned.
...
Geminicon will use dependency-free current verification scripts and a versioned source manifest, `.gemini/config.toml`.
